...
let blue = false;
let changeColor = document.getElementById('answer-one');
...
function taskSix() {
  document.getElementById('answer-six').innerHTML =
    '<b>Över 30: </b>' + overThirty + '<br>' +
    '<b>Under 30: </b>' + underThirty + '<br>' +
    '<b>Gift: </b>' + married + '</br>' +
...
const newLine = addMeSomeLineBreaks.replaceAll(',', ',<br/>');

function taskSeven() {
  document.getElementById('answer-seven').innerHTML = newLine;
}
...
function taskNine() {
  const date = new Date();
  const getColor = document.getElementsByClassName('answer-container');

  for (let i = 0; i < getColor.length; i++) {
    if (date.getHours() >= 17) {
      getColor[i].style.backgroundColor = 'blue';
    } else {
      getColor[i].style.backgroundColor = 'red'
    }
  }
}
